...
"""Generate CFD meshes FAST: Gmsh STEP→mesh, coarse scale, no optimization."""
import json
import subprocess
import os
...
class FastCFDGenerator:
    ...
    def generate_mesh_fast(self, node_id: str, idx: int, total: int, geometry_ref: str) -> dict:
        """Generate Gmsh mesh FAST: coarse scale, skip optimization."""
        ...
        try:
            ...
            # FAST: Use 0.5 scale (coarse) instead of 0.05 (very fine)
            # This reduces mesh from 50MB to ~5MB and speeds up 10x
            mesh_scale = 0.5 * (1.0 + mach * 0.05)
            
            msh_file = case_dir / 'domain.msh'
            
            # Gmsh: coarse mesh, NO optimization (-optimize skipped)
            result = subprocess.run(
                ['gmsh', str(step_path), '-3', '-format', 'msh2', '-o', str(msh_file),
                 '-clscale', str(mesh_scale), '-nopopup'],
                capture_output=True,
                text=True,
                timeout=30,  # Reduced from 120s
            )
            
            if not msh_file.exists() or msh_file.stat().st_size < 5000:
                return {'node_id': node_id, 'success': False, 'flow': flow_type}
    ...
    def run_all(self, graph_path: Path):
        ...
        print(f"Fast mesh generation: {len(parts)} STEP→Gmsh (coarse, no opt)...\n")
        
        successful = 0
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(
                    self.generate_mesh_fast,
                    node['id'],
                    i,
                    len(parts),
        ...
        print(f"\n✓ {successful}/{len(parts)} cases complete")

if __name__ == '__main__':
    gen = FastCFDGenerator(Path('artifacts/solver-cases-full'), max_workers=8)
