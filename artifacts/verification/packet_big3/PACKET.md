...
| skin stress at that station | axial 45.5 + bending 12.8 = **58.3 MPa** on a 1.87 mm wall |
| that wall | sized here for the flight loads at 446 mm radius, driven by **buckling under combined axial and bending** -- not carried over from the coupons, which are built at a clamped radius |
| margin against 188 MPa allowable | 3.22 |
| load set closes | shear -5.54e-12 N, moment 7.93e-01 N m -> **True** |

**The wall was thickened for buckling**, 1.50 to 1.87 mm. Sizing it for axial load alone left the shell unstable once the assembly bending moment was applied; nothing in the component checks would have shown that, since each part passes its own stress test.


That costs about **14.0 kg** over the barrel at 446 mm radius. The geometry below is still drawn at the component wall and the mass budget still charges that, so this figure is owed rather than paid -- subtract it from the slack before believing the budget closes.
...
| quantity | value |
|---|---|
| radius over thickness | 238 |
| classical critical stress | 176.8 MPa (a perfect cylinder; no real shell reaches it) |
| knockdown, compression / bending | 0.442 / 0.548 (NASA SP-8007) |
| allowable, compression / bending | 78.2 / 96.8 MPa |
| interaction R_c + R_b | 0.71 (must not exceed 1) |
| buckling margin | **1.40**, governed by compression |
| yield margin for comparison | 3.2 |

Buckling governs. The yield margin of 3.2 overstates the real one by a factor of 2.3, and every component margin in the table above is a yield margin computed the same way.
...
| quantity | value |
|---|---|
| first elastic bending mode | **29.2 Hz** free-free |
| next modes | 81.9 Hz, 160.3 Hz, 263.8 Hz |
| rigid-body modes found | 2 (a free planar beam has exactly 2) -> **True** |
| section | 1.87 mm wall at 446 mm radius, E 69 GPa |

Control bandwidth has to sit well below 29.2 Hz -- the usual allowance is a factor of five to ten -- or the autopilot drives the structure instead of steering it. This packet does not size a control system, so that comparison is left open rather than claimed as satisfied.
...
|---|---|---|---|---|
| stage 1 | 90% | 2.15 Hz | 130 kg (6% of liquid) | ratio 0.073 |
| stage 1 | 50% | 2.15 Hz | 130 kg (10% of liquid) | ratio 0.073 |
| stage 1 | 20% | 2.14 Hz | 129 kg (25% of liquid) | ratio 0.073 |

Closest approach to the first bending mode is a ratio of 0.073 -- separated by more than an octave.
...
| check | result | detail |
|---|---|---|
| skin buckling | **PASS** | interaction 0.71, margin 1.40 governed by compression |
