...
> The skin reaches 863 K, past the 450 K at which aluminium keeps useful strength. Every allowable in the component table below is a room-temperature value, so those margins do not hold at this condition: the vehicle needs a thermal protection system, a different skin material, or a trajectory that spends less time fast in thick air. This is a radiation-equilibrium steady state and the vehicle passes through quickly, so it is an upper bound rather than what the structure actually reaches -- but it is far enough past the limit to matter.
## Component verification

| component | load case | load | wall | driver | buckling margin | shell p95 | p99 | peak | 1st mode | mass | Izz | status |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| nose cone | drag plus normal force at max-Q, 1.5 ultimate | 331 N | 0.80 mm | minimum gauge | 340.15x | 1.7 MPa | 2 | 2 MPa | 1822 Hz | 65 g | 1.13e-04 | PASS |
| thrust structure | engine thrust into the aft ring | 67415 N | 2.20 mm | strength | 14.81x | 166.9 MPa | 250 | 535 MPa | 4378 Hz | 106 g | 1.77e-04 | PASS |
| fin set | one fin's share of fin normal force at 5 deg, 1.5 ultimate | 200 N | 0.80 mm | minimum gauge | 563.14x | 14.0 MPa | 28* | 86 MPa | 278 Hz | 128 g | 3.25e-04 | PASS |
| stage 1 tank | carries 973.9 kg propellant at 6.6 g | 117774 N | 3.85 mm | strength | 27.42x | 148.0 MPa | 212 | 556 MPa | 1969 Hz | 467 g | 7.51e-04 | PASS |
| interstage 1/2 | transmits lower-stage thrust to the stage above | 117774 N | 3.85 mm | strength | 27.42x | 152.4 MPa | 201 | 274 MPa | 4513 Hz | 225 g | 3.62e-04 | PASS |
| stage 2 tank | carries 253.2 kg propellant at 5.1 g | 24214 N | 0.86 mm | strength | 5.54x | 134.7 MPa | 139 | 173 MPa | 1085 Hz | 100 g | 1.45e-04 | PASS |
| interstage 2/3 | transmits lower-stage thrust to the stage above | 24214 N | 0.86 mm | strength | 5.54x | 136.6 MPa | 154 | 333 MPa | 2709 Hz | 48 g | 7.01e-05 | PASS |
| stage 3 tank | carries 65.8 kg propellant at 3.5 g | 4981 N | 0.80 mm | minimum gauge | 23.36x | 33.4 MPa | 35 | 40 MPa | 1114 Hz | 85 g | 1.03e-04 | PASS |
| interstage 3/4 | transmits lower-stage thrust to the stage above | 4981 N | 0.80 mm | minimum gauge | 23.36x | 32.8 MPa | 39 | 72 MPa | 2613 Hz | 41 g | 4.97e-05 | PASS |
| stage 4 tank | carries 23.1 kg propellant at 3.2 g | 1788 N | 0.80 mm | minimum gauge | 66.21x | 12.1 MPa | 13 | 17 MPa | 1242 Hz | 77 g | 7.60e-05 | PASS |

## Coupon stack (what was analysed)

The six analysed parts stacked nose-forward: 1.289 m in 10 sections, 1341 g of structure, centre of gravity 0.462 m from the aft end, Ixx 1.456e-01 and Izz 2.345e-03 kg m^2.

These are **coupons, not the vehicle**. Body radius is clamped to 50 mm so the parts stay meshable, while this mission's reference diameter is 670 mm -- a factor of 8.0 in radius, so the coupons carry 1341 g against the 466 kg of structure the planner sized. The stresses and modes above are about representative sections; the mass properties that describe the flight vehicle are below.
...
| aerodynamic side load | nose 5.4 kN + fins 18.6 kN |
| peak bending moment | 8.0 kN m at 1.79 m from aft (41% of length) |
| skin stress at that station | axial 24.5 + bending 28.2 = **52.7 MPa** on a 0.80 mm wall |
| margin against 700 MPa allowable | 13.28 |
| load set closes | shear -1.27e-11 N, moment 4.26e-01 N m -> **True** |
...
| interaction R_c + R_b | 0.44 (must not exceed 1) |
| buckling margin | **2.25**, governed by compression |
| yield margin for comparison | 13.3 |

Buckling governs. The yield margin of 13.3 overstates the real one by a factor of 5.9, and every component margin in the table above is a yield margin computed the same way.
...
| aerodynamic moment about the CG | 17.7 kN m |
| thrust x gimbal arm | 79.7 kN x 1.86 m |
| gimbal deflection required | **6.8 deg** |
| assumed available | 8.0 deg (typical production engine, not a hardware specification) |
| authority | **True** (utilisation 0.85) |
...
| frequency | value |
|---|---|
| rigid-body pitch (weathercock) at max-Q | 1.54 Hz |
| stage 1 slosh at max-Q | 2.48 Hz |
| ratio | **1.61** |
| first bending | 49.5 Hz |
| usable control band | No usable band: flying the vehicle needs at least 4.62 Hz while the slosh caps bandwidth at 0.50 Hz. The slosh lies inside the required band rather than above it, so added damping cannot open it: this needs a notch filter at that frequency, hardware that moves the mode, or an autopilot designed to fly through it. |
| lowest flexible mode / rigid-body mode | **1.61** |
| verdict robust to the separation rules | **True** |

The verdict holds across every rigid-body margin from 1.5 to 3 and every flexible separation from 1.5 to 5, so it is a property of the vehicle rather than of the factors chosen. The lowest flexible mode sits at 1.61 times the rigid-body mode; below about 3 there is no bandwidth that dominates one without exciting the other.


**Baffles cannot close this.** the control bandwidth this vehicle needs (4.62 Hz) is above its slosh frequency (2.48 Hz), so the mode lies inside the control band rather than above it. Baffles add damping but do not move the frequency, so no baffle closes this. It needs a notch filter at the slosh frequency, a tank that sloshes elsewhere, or an autopilot designed to fly through the mode
...
|---|---|---|---|---|
| 1.50 cal | 892 mm | 13.69 /rad | 14.47 deg | no |
| 1.40 cal | 800 mm | 12.06 /rad | 11.86 deg | no |
| 1.30 cal | 726 mm | 10.78 /rad | 9.82 deg | no |
| 1.20 cal | 666 mm | 9.74 /rad | 8.18 deg | no |
| 1.10 cal | 615 mm | 8.89 /rad | 6.83 deg | yes |

Fins resized for 1.10 calibers of static margin, down from 1.50, so the engine can trim the vehicle at max-Q within 8.0 degrees.
...
| quantity | value |
|---|---|
| fin span (each of 4) | 615 mm |
| fin root / tip chord | 670 / 335 mm, sweep 402 mm |
| centre of pressure | 1.124 m from aft |
| centre of gravity | 1.861 m from aft |
| static margin | 1.10 calibers (target 1.1, sized for liftoff) |
| normal force slope | nose 2.00 + fins 6.89 = 8.89 /rad |

| burn state | vehicle mass | centre of gravity | static margin |
|---|---|---|---|
| liftoff | 1806.6 kg | 1.861 m | 1.10 cal |
| stage 1 burnout | 832.8 kg | 2.454 m | 1.98 cal |
...
| fin 1 | extrude | 0.167 m | 670 mm | 10.26 kg |
| fin 2 | extrude | 0.167 m | 670 mm | 10.26 kg |
| fin 3 | extrude | 0.167 m | 670 mm | 10.26 kg |
| fin 4 | extrude | 0.167 m | 670 mm | 10.26 kg |
...
| term | mass |
|---|---|
| skin, as drawn | 125.6 kg |
| engine, from liftoff thrust at T/W 60 | 88.1 kg |
| accounted for | 213.7 kg |
| structural budget | 465.6 kg |
| slack | +251.9 kg |

> The budget holds, with 251.9 kg left for plumbing, avionics, tank domes and separation hardware -- none of which the geometry draws.
...
| check | result | detail |
|---|---|---|
| skin buckling | **PASS** | interaction 0.44, margin 2.25 governed by compression |
| slosh / pitch-mode separation | **PASS** | slosh 2.48 Hz against pitch 1.54 Hz, ratio 1.61 |
| control bandwidth window | **FAIL** | needs >= 4.62 Hz, capped at 0.50 Hz by slosh |
| thrust vector control authority | **PASS** | 6.8 deg required against 8.0 available |
...
Allowable 700 MPa, derived from inconel-718 at 1030 MPa (typical (non-statistical)) with a yield factor of safety of 1.25 and a 0.85 knockdown. All 10 components passed: **True**. Assembly checks passed: **False**. Overall: **False**
