...
def _case_dirs(
    *,
    ...
    if cases_file is not None:
        ...
        dirs = [FEA_DIR / name for name in names if (FEA_DIR / name).is_dir()]
    else:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional text file of case directory names to process",
    )
    parser.add_argument(
        "--setup-only",
        action="store_true",
    ...
    print("=" * 80)
    print(
        f"Cases: {len(case_dirs)} (resume={args.resume} force={args.force} "
        f"cases_file={args.cases_file})",
        flush=True,
    )
