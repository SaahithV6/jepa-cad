...
| fin set | one fin's share of fin normal force at 5 deg, 1.5 ultimate | 200 N | 0.80 mm | minimum gauge | 563.14x | 12.5 MPa | 26* | 264 MPa | 336 Hz | 362 g | 8.53e-04 | PASS |
...
13 of 13 internal cross-checks agree. These compare numbers the packet already reports against each other -- gross mass against the stack it lists, the structural coefficient against the stage masses, and every component's mass over its volume against the density of the alloy the design selected. Five defects in this packet were of exactly that shape and every one was invisible to the tests, because each individual model was right and only the connection between them was wrong.
